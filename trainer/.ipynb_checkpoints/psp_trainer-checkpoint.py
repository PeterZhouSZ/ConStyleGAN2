...
class Trainer():
    def __init__(self, args, device):
        ...
        if self.args.adv_training:
            self.netD_local = Discriminator(args.data_size, in_c=4).to(device) 
    ...
    def prepare_optimizer(self):
        if self.args.opt == 'adam':
            self.optimizerE = optim.Adam( self.encoder.parameters(), lr=self.args.lr, betas=(0, 0.99) )
            if self.args.adv_training:
                self.optimizerD_local = optim.Adam( self.netD_local.parameters(), lr=self.args.lrD, betas=(0, 0.99) )
                self.optimizerD_global = optim.Adam( self.netD_global.parameters(), lr=self.args.lrD, betas=(0, 0.99) )
        elif self.args.opt == 'ranger':
            self.optimizerE = Ranger( self.encoder.parameters(), lr=self.args.lr  )
            if self.args.adv_training:
                self.optimizerD_local = Ranger( self.netD_local.parameters(), lr=self.args.lrD   )
                self.optimizerD_global = Ranger( self.netD_global.parameters(), lr=self.args.lrD   )
        else:
            assert False
    ...
    def trainD(self):
        ...
        real_pred = self.netD_local(   torch.cat([augmented_real_img,self.real_seg],1)   )
        fake_pred = self.netD_local(   torch.cat([augmented_fake_img,self.real_seg],1)   )        
        d_loss = self.d_logistic_loss(real_pred, fake_pred) * self.args.D_local_lambda
        ...
        d_global_loss = d_logistic_loss(real_pred, fake_pred) * self.args.D_global_lambda       
    ...
    def trainE(self):
        ...
        if self.args.adv_training:
            ...
            fake_pred = self.netD_local(  torch.cat([augmented_fake_img,self.real_seg],1)  )
            g_loss = self.g_nonsaturating_loss(fake_pred) * self.args.G_local_lambda
            self.loss_dict["g"] = g_loss.item()

            fake_pred = self.netD_global(augmented_fake_global)
            g_global_loss = self.g_nonsaturating_loss(fake_pred) * self.args.G_global_lambda
