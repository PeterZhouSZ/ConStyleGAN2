...
from models.scenegan_refiner import Generator, Discriminator
from dataset.image_dataset import get_refiner_dataloader
...
def process_data(args, data, device):  
    ...
    out['composition'] = data['composition']

    # shift by 1 if needed. 
    # In refiner training we do not do zero padding on semantic
    ...
    out['global_pri'] = torch.cat( [data['composition'], data['scene_sem'], get_edges(data['scene_ins'])], dim=1 )
...
class Trainer():
    ...
    def prepare_dataloader(self):
        self.train_loader =  sample_n_data( self.args, get_refiner_dataloader(self.args, train=True))   
        self.test_loader = sample_n_data( self.args,  get_refiner_dataloader( self.args, train=False ) )
    ...
    def prepare_visualization_data(self):
        ...
        self.image_train_saver( self.train_sample['composition'], 'composition.png' )   
        ...
        self.image_test_saver( self.test_sample['composition'], 'composition.png' ) 
    ...
    def regularizeVGG(self):
        ...
        vgg_loss = self.get_vgg_loss(fake_img, self.data['composition']) * self.args.vgg_reg_every * self.args.vgg_regularize
