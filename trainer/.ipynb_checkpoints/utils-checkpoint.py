...
print('blur is bilinear!!!!!!!!!!!!!!!!!!!!!')

def blur( image, factor, mask=None ):
    """
    Image is a tensor BS*3*H*W
    mask should be a binary mask BS*1*H*W indicating which region used to blur. (0 means remian intack)
    factor is a downsamplefactor. For example, 4 means H, W -> H/4, H/4
    """
    
    H, W = image.shape[2:]
    # except for celeba, all the other instances are using bilinear
    blur_image = F.interpolate( image, (H//factor,W//factor), mode='bilinear' )  # bilinear
    blur_image = F.interpolate( blur_image, (H,W), mode='bilinear' )   # bilinear
    if mask == None:
        return blur_image
    else:
