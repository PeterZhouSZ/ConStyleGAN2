...
class Generator(nn.Module):
    def __init__(self, args, channel_multiplier=2, blur_kernel=[1, 3, 3, 1], lr_mlp=0.01, ):
        super().__init__()
    ...
    def __process_global_pri(self, global_pri, styles, input_type):
        "Get z code from global pri if input style is not given"
        loss = None 
        if input_type == None:
            z, loss = self.encoder(global_pri)
            styles = [z]
            input_type = 'z'
        return styles, input_type, loss
    ...
    def forward(self, global_pri, target_mask, styles=None, return_latents=False, inject_index=None, truncation=1, truncation_latent=None, input_type=None, noise=None, randomize_noise=True, return_loss=True):
        ...
        target_mask: a binary tensor (BS*1*H*W) specifying fg object shape.
        ...
        return_loss: if return kl loss. Note that if you give your own z or w code then klloss will be None 
        ...
        styles, input_type, loss = self.__process_global_pri(global_pri, styles, input_type)
        ...
        for conv1, conv2, noise1, noise2, to_rgb in zip( self.convs[::2], self.convs[1::2], noise[1::2], noise[2::2], self.to_rgbs ):
            out = out * F.interpolate(target_mask, size=out.size()[2:], mode='bilinear')
