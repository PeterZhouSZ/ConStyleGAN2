...
class Generator(nn.Module):
    def __init__(self, args, device, blur_kernel=[1, 3, 3, 1], lr_mlp=0.01, ):
        super().__init__()
        print('This is a residual refiner')
        #print('This is a normal refiner')
        ...
        self.log_size = int(math.log(args.scene_size, 2)) - int(math.log(self.args.starting_feature_size_bg, 2))
        ...
        expected_out_size = self.args.starting_feature_size_bg
        layer_idx = 0 
        ...
        in_channel = self.channels[self.args.starting_feature_size_bg]
        expected_out_size = self.args.starting_feature_size_bg      
        for _ in range(self.log_size):  
            expected_out_size *= 2 
            out_channel = self.channels[expected_out_size]
            if self.args.modify_feature_bg == 'concat':
                self.convs.append( StyledConv( in_channel+self.args.number_of_semantic+1, out_channel, 3, args.style_dim, upsample=True, blur_kernel=blur_kernel ) )
                self.convs.append( StyledConv(out_channel+self.args.number_of_semantic+1, out_channel, 3, args.style_dim, blur_kernel=blur_kernel) )
            else:
    ...
    def modify_feature(self, x, global_pri):

        if self.args.modify_feature_bg == 'none':
            return x 
        elif self.args.modify_feature_bg == 'concat':
            return  torch.cat(  [x, F.interpolate(global_pri, size=x.size()[2:])], dim=1  )
        else:
    ...
    def forward(self, global_pri, styles=None, return_latents=False, inject_index=None, truncation=1, truncation_latent=None, input_type=None, noise=None, randomize_noise=True, return_loss=True):
        ...
        for conv1, conv2, noise1, noise2, to_rgb in zip( self.convs[::2], self.convs[1::2], noise[::2], noise[1::2], self.to_rgbs ):
            out = self.modify_feature(out, global_pri)
        ...
        image = F.tanh(skip+global_pri[:,0:3,:,:])
        #image = F.tanh(skip)
...
class Discriminator(nn.Module):
    def __init__(self, args, blur_kernel=[1, 3, 3, 1]):
        ...
        log_size = int( math.log(args.scene_size,2) )
        convs = [ ConvLayer(3, channels[args.scene_size], 1) ]        

        in_channel = channels[args.scene_size]
...
class Encoder(nn.Module):
    def __init__(self, args, blur_kernel=[1, 3, 3, 1]):
        ...
        in_c = args.number_of_semantic 
        self.convs1 = ConvLayer(3+in_c+1, channels[args.scene_size], 1)  # this 1 is edge map
        ...
        for i in range(log_size, 2, -1):
            ...
            if 4 <= out_size <= args.starting_feature_size_bg: 
    ...
    def forward(self, input):
        ...
        for conv in self.convs2:
            out = conv(out)
            if 4 <= out.shape[3] <= self.args.starting_feature_size_bg:
