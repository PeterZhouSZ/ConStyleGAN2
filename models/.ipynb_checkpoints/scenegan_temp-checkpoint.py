...
"THIS CODE IS FROM ../branch1/models/scenegan.py"
...
class Generator(nn.Module):
    def __init__(self, args, device, blur_kernel=[1, 3, 3, 1], lr_mlp=0.01, ):
        ...
        self.log_size = int(math.log(args.data_size, 2)) - int(math.log(self.args.starting_feature_size, 2))
        ...
        expected_out_size = self.args.starting_feature_size
        layer_idx = 0 
        for _ in range(self.log_size):
            expected_out_size *= 2
            shape = [1, 1, expected_out_size, expected_out_size]
            self.noises.register_buffer(f'noise_{layer_idx}', torch.randn(*shape))
            self.noises.register_buffer(f'noise_{layer_idx+1}', torch.randn(*shape))
            layer_idx += 2 


        in_channel = self.channels[self.args.starting_feature_size]
        expected_out_size = self.args.starting_feature_size      
        for _ in range(self.log_size):  
            expected_out_size *= 2 
            out_channel = self.channels[expected_out_size]
            if self.args.modify_feature == 'concat':
                self.convs.append( StyledConv( in_channel+1, out_channel, 3, args.style_dim, upsample=True, blur_kernel=blur_kernel ) )
                self.convs.append( StyledConv(out_channel+1, out_channel, 3, args.style_dim, blur_kernel=blur_kernel) )
            else:
        ...
        self.to_mask = nn.Sequential(  nn.Conv2d(out_channel,1,3,1,1), nn.Sigmoid() )


        self.detector = EdgeDetector()
        if self.args.modify_feature == 'interpolate':            
            self.interpolator_container = { 4: FeatureInterpolator(4).to(device),
                                            8: FeatureInterpolator(8).to(device),
                                            16: FeatureInterpolator(16).to(device),
                                            32: FeatureInterpolator(32).to(device),
                                            64: FeatureInterpolator(64).to(device),
                                            128: FeatureInterpolator(128).to(device),
                                            256: FeatureInterpolator(256).to(device),
                                            512: FeatureInterpolator(512).to(device) }
        if self.args.modify_feature == 'propagate':
    ...
    def __prepare_starting_feature(self, global_pri, styles, input_type):
        ...
        start = self.args.starting_feature_size // 2 
        end = self.args.starting_feature_size // 2  + self.args.starting_feature_size
    ...
    def modify_feature(self, x, mask):
        ...
        if self.args.modify_feature == 'none' or mask == None:
            return x 

        elif self.args.modify_feature == 'concat':
            return  torch.cat(  [x, F.interpolate(mask, size=x.size()[2:])], dim=1  )

        elif self.args.modify_feature == 'interpolate':
            featureinterpolator = self.interpolator_container[ x.shape[1] ]
            mask = F.interpolate(mask, size=x.size()[2:] )
            edge = self.detector( mask, self.args.interpolator_region  )
            return  featureinterpolator(x, edge) 

        elif self.args.modify_feature == 'propagate':
            ...
            assert False, 'not supported modify_feature type' 
...
class Encoder(nn.Module):
    def __init__(self, args, blur_kernel=[1, 3, 3, 1]):
        ...
        break_idx_map = {4:6, 8:5, 16:4, 32:3}
        break_idx = break_idx_map[args.starting_feature_size]
        self.convs1 = nn.Sequential(*convs[0:break_idx])
        self.convs2 = nn.Sequential(*convs[break_idx:])
    ...
    def forward(self, input):
        ...
        feature = self.convs1(input)
        out = self.convs2(feature)
